...
최종 갱신: 2026-09-20 (IPS Discover 페이지 확장) / Claude Code
...
- 진행 방식 다이아몬드를 정사각형 다이아몬드 두 개(첫 번째가 큼)로 바꾸고, 네 단계에 마우스를 올리거나 포커스하면 오른쪽에 세부 내용이 보인다. 색은 기존 잉크 한 가지.
- Discover 구성을 사용자 지시대로 재배치: 02 서비스 블루프린트(디스코드 저널 채널·Phi LMS TO-DO 기준 6단계 × 4레인, 불편 위치 표시, 열 호버) → 03 직접 겪은 불편 → 04 정량 조사(논문 A·B + 서비스 사례 C) → 05 정성 조사(동료 인터뷰, 9월 21일 예정) → 06 근거를 숫자로 → 07 불편 × 조사 근거 → 08 결론.
- 인터뷰 결과는 `ips/ips.js`의 `INTERVIEWS`·`INTERVIEW_STATS` 배열에 넣으면 카드와 수치가 채워진다(비어 있으면 점선 자리 표시).
- 내 기록 수치(사용자 제공 스크린샷 기준): LMS TO-DO 전체 130(할 일 88·완료 42), 9/15~17 저널 3편에 5과목, 9/16 BI Findings 9개. 셀프 피드백 미확인 칸 수는 집계 예정.
- 후속 수정(같은 날): 두 다이아몬드 크기 통일, 블루프린트의 접점과 지원 시스템을 한 레인으로 합침, 호버 시 다른 칸을 흐리게 하지 않고 올린 칸만 살짝 커지게(블루프린트 칸·비교표 축), 직접 겪은 불편을 A·B 두 박스 + 하단 1·2·3 목록으로 바꿈.
- 후속 수정 2: 블루프린트 아래 한 줄 Insight, 논문 카드는 번호·주장·논문 제목만(호버 시 핵심 내용 제목이 하나씩), 논문 패널은 제목·핵심 내용(굵게 강조)·Insight·도식화·원문 링크로 재구성(주의 삭제). 주소 뒤 #paper-01~05로 해당 논문 패널을 바로 열 수 있다.
...
### Assignment Manage: 즐겨찾기한 과제는 어느 주차를 보든 늘 표 맨 위에 (2026-09-19, 사용자 지시 · Claude Code)

- 다른 주차에서 즐겨찾기한 과제도 지금 보는 주차 표 맨 위(즐겨찾기 묶음)에 **늘** 보인다. 순서는 누른 순서.
  `loadWeek`가 이번 주 표와 함께, 즐겨찾기에 걸린 다른 주차들의 표(`/weeks/:n/matrix`, 주차마다 한 번)를 받아 그 줄만 `pinnedRows`로 둔다.
  읽기 전용(로그인 전)이면 상태 파일(`snapshotMatrix`)에서 가져온다.
- 같은 과목이 두 줄일 수 있다(예: 위에 AL WK01 즐겨찾기, 아래에 이번 주 AL WK02). 다른 주차 줄의 WK 박스는 검정 글씨·회색 테두리(`.am-week-tag.is-other`).
  그 줄 별표를 풀면 줄이 표에서 사라진다(그 주차 표에는 그대로 있음).
- 줄마다 자기 주차를 가진다: 줄 키 `data-am-row="AL:1"`, 자세히보기 버튼 `data-note-wk`, 별표 `data-am-week`.
  과제 내용 팝업(`noteState.week`)은 **그 줄의 주차** 공지를 열고, 수정·추가·삭제도 그 주차에 저장한다. 저장 뒤 표는 보고 있던 주차를 다시 불러온다
  (다른 주차 즐겨찾기 줄에서 저장해도 표가 그 주차로 넘어가지 않는다). 공지가 다른 주차라 옮겨 저장할 때만 그 주차 표로 간다(예전과 같음).
  주차 고르기 버튼 문구 "(보고 있는 주)" → "(이 칸)". 상태 상세 팝업의 지각 판정도 그 대상의 주차 줄 공지로.
- `home.html`: `assignment.js?v=20260919-6`, `phi-brain.css?v=20260919-7`.
- 검증(로컬): 읽기 전용 표 — 즐겨찾기 ["VT:1","BI:2"]일 때 2주차 표 맨 위 VT(WK01, 다른 주차 표시)·BI, 13줄; 1주차로 가면 VT·BI(WK02 표시);
  BI:2 별표를 풀면 사라지고 저장값 ["VT:1"]. 가짜 로그인·가짜 API 임시 페이지(확인 후 삭제) — 2주차 표 위 AL:1의 자세히보기가 "1주차" 공지로 열림,
  거기서 추가하면 1주차에 저장(baseVersion 1)되고 표는 2주차 그대로·팝업 유지, 이번 주 AL:2는 2주차 공지, AL:1 더블클릭 → 1주차 전체보기. 실제 서버로는 확인 못 함.

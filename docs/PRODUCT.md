...
- **행동은 펜, 마감은 시계, 삭제는 ✕(2026-09-13 확정, 처음엔 더블클릭만이었다가 같은 날 아이콘 3개로 교체):**
  행동 문장 우측에 수정(펜)·마감(시계)·삭제(✕) 아이콘이 이 순서로 항상 보인다. 기존 항목 ⋯ 메뉴(수정·마감·소속
  변경·원문 저널 열기·삭제)는 없앴다 — 소속 변경은 드래그로만 하고, "원문 저널 열기"는 대체 없이 없앴다(사용자 결정).
  마감이 없는 행동은 하단에 마감 표시가 아예 없다(예전엔 "+ 마감" 안내가 있었다 — 시계 아이콘이 그 역할을 대신함).
...
- 이번 범위 제외: 정리(AI) 결과·검토 상태 표시, 검색·페이지네이션·월별 그룹핑·삭제, 기기 간 동기화.

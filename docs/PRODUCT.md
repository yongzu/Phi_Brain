...
- **소속은 항목당 하나:** 특정 과목 / General / 임시(미지정). General은 "특정 과목이 아님"으로 분류 완료된 상태,
  임시는 아직 분류하지 않은 상태 — 둘을 합치지 않으며 가짜 과목으로 취급하지 않는다.
- 이 원칙은 Future Item에만 적용한다. 저널·인사이트의 복수 과목 연결 정책은 바꾸지 않는다.
- 과목 약어·순서는 기존 과목 목록 하나를 기준으로 통일(EWA 같은 다른 표기는 별도 과목으로 만들지 않음).
- 이번 범위 제외: 마감일, 반복 일정, 하위 할 일, 알림, AI 분류.
...
## Assignment Manage — 요구사항 정리, 미구현

- 주차별 12개 과목의 과제·셀프피드백 제출 확인과 수업 보드·제출폼 바로가기.
- 셀프피드백을 저널에 가져오지 않는다. Future Item 완료 여부와 제출 상태를 연결하지 않는다.
- Gmail 응답 사본으로 자동 확인하는 방식 검토: 과목·본인·트랙·본문 주차·제출 종류를 식별한다.
- 확인된 표본은 Engaging with AI 과제 응답 사본이며, 본문에 2주차·트랙 및 제출물 링크가 포함된다.
- 다른 과목과 셀프피드백 형식은 아직 미검증이다. 메일 수신일을 대상 주차로 사용하지 않는다.
- 제안 상태: 미확인 / 메일로 제출 확인 / 직접 확인 / 해당 없음. 메일 부재·동기화 실패를 미제출로 단정하지 않는다.
- OAuth·Gmail API·서버 저장·실제 메일 파싱 검증은 아직 없다. 관련 상세 구현 지시 초안은 대화에서 작성했으나, 구현 완료를 뜻하지 않는다.

...
- **행동·마감은 더블클릭으로 수정, 항목 메뉴는 없앰(2026-09-13 확정):** 행동 문장과 마감을 각각 더블클릭하면 바로
  수정 상태가 된다. 기존 항목 ⋯ 메뉴(수정·마감·소속 변경·원문 저널 열기·삭제)는 없앴다 — 소속 변경은 드래그로만
  하고, 삭제는 행 우측의 작은 ✕ 버튼으로 남기고, "원문 저널 열기"는 대체 없이 없앴다(사용자 결정).
- **즐겨찾기 별표(2026-09-13 확정):** 과목/커스텀 박스마다 우측 상단에 항상 별(★)이 있고, 즐겨찾기하면 `#333333`
  (`--ink`)으로 칠해진다. Journal Archive의 과목별 카드도 같은 별표로 즐겨찾기한다.
